...
## *タイムライン上*のホーム/鍵/ダイレクト投稿の背景色をそれぞれ緑/赤/青にする
最終更新日: 2026-04-22
~~~css
/* Home, note and reply, on timeline */
.xBwhh:has(.ti-home),
.x5yeR:has(.ti-home),
.xD2mX.xCFgx:has(.ti-home) {
    background-color: rgba(0, 135, 68 , 0.10) !important;
}
/* Follower only, note and reply, on timeline */
.xBwhh:has(.ti-lock),
.x5yeR:has(.ti-lock),
.xD2mX.xCFgx:has(.ti-lock) {
    background-color: rgba(214, 45, 32, 0.10) !important;
}
/* Direct, note and reply, on timeline */
.xBwhh:has(.ti-mail),
.x5yeR:has(.ti-mail),
...
### changelog
* 2026-03-31 初版
* 2026-04-20 リノートの背景色を修正
* 2026-04-22 コメントの修正・並びの修正 (内容に変更無し)

### 既知のバグ
* リノートのスマート省略がONになっているとき，一部の背景色がデフォルトのまま
  * CSSじゃどうしようもなさそうなので我慢してください

## *詳細画面*{: .b}上のホーム/鍵/ダイレクト投稿の背景色をそれぞれ緑/赤/青にする
最終更新日: 2026-04-22
~~~css
/* Home, note and reply, on details */
.xexC6:has(.ti-home),
.xDmvd.xD2mX:has(.ti-home),
.xD2mX.x8Lf4:has(.ti-home),
.xtE7t:has(.ti-home) {
    background-color: rgba(0, 135, 68 , 0.10) !important;
}
/* Follower only, note and reply, on details */
.xexC6:has(.ti-lock),
.xDmvd.xD2mX:has(.ti-lock),
.xD2mX.x8Lf4:has(.ti-lock),
.xtE7t:has(.ti-lock) {
    background-color: rgba(214, 45, 32, 0.10) !important;
}
/* Direct, note and reply, on details */
.xexC6:has(.ti-mail),
.xDmvd.xD2mX:has(.ti-mail),
.xD2mX.x8Lf4:has(.ti-mail),
.xtE7t:has(.ti-mail) {
	background-color: rgba(0, 87, 231, 0.10) !important;
}
~~~
